...
if (!ownerAndRepo) {
  cliError(`The first argument must be :owner/:repo (like "gulpjs/gulp") but it was empty.`);
}
...
if (ownerAndRepo.includes('/')) {
  ...
  if (split.length > 2) {
    cliError(`We encountered more than 1 "/" in the first argument. We were expecting :owner/:repo but you gave "${ownerAndRepo}".`);
  }
  ...
  if (!owner) {
    cliError(`It looks like you specified an empty :owner in :owner/:repo. You gave "${ownerAndRepo}".`);
  }
  ...
  cliError(`The first argument must be :owner/:repo (like "gulpjs/gulp") but you gave "${ownerAndRepo}".`);
}
